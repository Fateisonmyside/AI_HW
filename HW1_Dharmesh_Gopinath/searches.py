...
def next_state(state, action):  ## the results of each action   #################################################
        ...
        for action in range(1, 5):
            ...
            if (child.map not in closed_states):
                if (child.state == goal_state):
                    print("Success! Number of iteration =", counter, ", The size of closed states:",
                          len(closed_states))
                    Goal = True
                    break
                else:
            ...
            counter += 1
            if (counter % 100000 == 0):
                print("Iterations: ", counter, ", The size of closed states:", len(closed_states))
        if Goal == True:
            break
...
def depth_first_search(intial_state, goal_state):
    open_states = deque([State(intial_state, None, None, 0, 0)])
    closed_states = set()
    ...
    while (len(open_states) != 0):
        ...
        if (len(open_states) == 0):
            print("All of the open states are consumed and there is no possible solution.")
    if Goal == True:
        return child
...
def main(InitialState, GoalState, SearchType):
    print("Initial State: ", InitialState)
    print("Goal State: ", GoalState)
    if SearchType == 'bfs':
        AchievedGoal = breadth_first_search(InitialState, GoalState)
    elif SearchType == 'dfs':
        AchievedGoal = depth_first_search(InitialState, GoalState)
    elif SearchType == 'ucs':
...
SearchType = 'bfs'  # "bfs", "dfs", or "ucs"

if __name__ == "__main__":
    main(InitialState, GoalState, SearchType)